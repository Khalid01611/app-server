...
import { SMSTemplate, InvoiceSettings, SiteSettings } from "../../models/Settings";
...
// Site Settings
export const createOrUpdateSiteSettings = async (req: Request, res: Response): Promise<Response | any> => {
  try {
    const { siteName, mobile, email, address } = req.body;
    ...
    if (settings) {
      ...
      if (mobile) settings.mobile = mobile.trim();
    ...
    console.error("Error in createOrUpdateSiteSettings:", error);
    return res.status(500).json({
      success: false,
      message: "Internal server error",
      error: process.env.NODE_ENV === "development" ? error.message : "Something went wrong",
...
export const getSiteSettings = async (req: Request, res: Response): Promise<Response | any> => {
  try {
